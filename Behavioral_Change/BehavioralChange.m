% This program solves the mathematical model of the manuscript "Modeling behavioral
% change and outbreak containment: a trade-off between lockdown and 
% compliance" using the 4th order Runge-Kutta method. We employ a csv file 
...
% incidence graphs, 2) histograms of the prevalence and incidence peaks
% (values and dates), 3) histograms of cumulative incidence at April 30th,
% and 4) files with data of the quantiles (prevalence peaks, incidence peaks, 
% cumulative incidence at April 30th and cumulative incidence at August 31st)
...
M = csvread('Poisson_Samples.csv',1,0);
...
q      = 0.8;   % Fraction of individuals isolated
...
for id = 1:size(M,1)
    ...
    %% Baseline parameters
    beta_ia = M(id,2);     % Contact rate for asymptomatic individuals
    beta_is = M(id,3);     % Contact rate for symptomatic individuals
    gamma   = 1/5.1;       % 1/gamma incubation period
    eta     = M(id,1);     % 1/eta infectious period
    rho_i   = 0.45;        % proportion of asymptomatic infectious individuals

    %% Basic reproductive number
    R0  = sqrt((rho_i*beta_ia + (1 - rho_i)*beta_is)*(s/(eta)));
...
quan_peak_prev_date  = timo(round(quantile(Num_Date_Peak_prev,[0.025, 0.5, 0.975])));      % Calculating the quantiles of dates of the prevalence peaks
quan_peak_in_s_date  = timo(round(quantile(Num_Date_Peak_in_s,[0.025, 0.5, 0.975])));      % Calculating the quantiles of the dates of the symptomatic incidence peaks
quan_peak_in_as_date = timo(round(quantile(Num_Date_Peak_in_as,[0.025, 0.5, 0.975])));     % Calculating the quantiles of the dates of the total incidence peaks
...
figure
subplot(1,2,1); hist(cum_is_30); title('Cumulative Incidence - Symptomatic (April 30th)')    % Histogram of cumulative symptomatic incidence at April 30th
subplot(1,2,2); hist(cum_ias_30); title('Cumulative Incidence (April 30th)')                 % Histogram of cumulative total incidence at April 30th  
